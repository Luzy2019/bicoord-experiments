...
class BimanualCouplingEstimator(nn.Module):
    """Auxiliary marginal/conditional denoisers for learned arm dependency."""
...
class CouplingScheduledDiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def _estimate_coupling(self, normalized_actions: torch.Tensor, global_cond: torch.Tensor) -> torch.Tensor:
        ...
        timesteps = torch.zeros(normalized_actions.shape[0], device=normalized_actions.device, dtype=torch.long)
    ...
    def compute_loss(self, batch):
        ...
        if self.coupling_estimator is not None and self.coupling_aux_loss_weight > 0:
            coupling_loss, coupling_logs = self.coupling_estimator.compute_aux_loss(
                noisy_trajectory[:, :, self.left_slice],
                noisy_trajectory[:, :, self.right_slice],
                timesteps,
                global_cond,
                target[:, :, self.left_slice],
                target[:, :, self.right_slice],
            )
