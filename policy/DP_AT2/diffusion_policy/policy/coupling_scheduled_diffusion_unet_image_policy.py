...
class CouplingScheduledDiffusionUnetImagePolicy(BaseImagePolicy):
    """Action-only DP with learned coupling-aware compact scheduling.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not obs_as_global_cond:
            raise ValueError("CouplingScheduledDiffusionUnetImagePolicy requires obs_as_global_cond=True")
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        compact_schedule_durations = torch.stack([item["durations"] for item in scored], dim=0)[best_idx, batch_idx]
        compact_schedule_ends = torch.stack([item["ends"] for item in scored], dim=0)[best_idx, batch_idx]
        compact_schedule_speed_scale = torch.stack([item["speed_scale"] for item in scored], dim=0)[best_idx, batch_idx]
    ...
    def compute_loss(self, batch):
        ...
        extra_logs = {"diffusion_loss": diffusion_loss}
