...
class LatentTraceWorldModel(nn.Module):
    """Latent action-time verifier used for compact trace search."""
...
class TraceEnergyHead(nn.Module):
    """Scalar feasibility energy for generated action-time traces."""
...
class ActionTimeFlowUnetImagePolicy(BaseImagePolicy):
    """Flow Matching policy over joint action-time traces.

    The generated object is Y=(A_L,A_R,t_L,t_R). Compact execution is represented
    by the time channels themselves, not by alpha scaling or explicit gates.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not obs_as_global_cond:
            raise ValueError("ActionTimeFlowUnetImagePolicy requires obs_as_global_cond=True")
    ...
    def _energy_contrastive_loss(
        ...
        negative[..., self.action_dim :] = torch.rand_like(negative[..., self.action_dim :])
    ...
    def _sample_trace(self, batch_size: int, global_cond: torch.Tensor, generator=None) -> torch.Tensor:
        ...
        for step_idx in range(num_steps):
            ...
            velocity = self.model(trace, t, global_cond=global_cond)
            trace = trace + dt * velocity
        return trace
    ...
    def compute_loss(self, batch):
        ...
        noise = torch.randn_like(target_trace)
        timesteps = torch.rand((batch_size,), device=target_trace.device, dtype=target_trace.dtype)
        t_view = timesteps.view(batch_size, 1, 1)
        noisy_trace = (1.0 - t_view) * noise + t_view * target_trace
        target_velocity = target_trace - noise
        pred_velocity = self.model(noisy_trace, timesteps, global_cond=global_cond)
        flow_loss = reduce(F.mse_loss(pred_velocity, target_velocity, reduction="none"), "b ... -> b (...)", "mean").mean()
        ...
        total_loss = flow_loss + self.monotonic_loss_weight * mono_loss + self.energy_loss_weight * energy_loss
