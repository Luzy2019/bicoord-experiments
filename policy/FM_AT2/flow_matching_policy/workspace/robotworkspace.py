...
from flow_matching_policy.dataset.base_dataset import BaseImageDataset
from flow_matching_policy.common.checkpoint_util import TopKCheckpointManager
from flow_matching_policy.common.json_logger import JsonLogger
from flow_matching_policy.common.pytorch_util import dict_apply, optimizer_to
from flow_matching_policy.model.diffusion.ema_model import EMAModel
from flow_matching_policy.model.common.lr_scheduler import get_scheduler
from flow_matching_policy.env_runner.rl_rollout_runner import RLRolloutRunner
...
class RobotWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        training_stage = str(cfg.training.get("stage", "bc")).lower()
        rl_cfg = cfg.get("rl", {})
        rl_enabled = bool(rl_cfg.get("enabled", False)) or training_stage in ("rl", "joint")
        rl_runner = None
        if rl_enabled:
            if isinstance(rl_cfg, dict):
                rl_cfg["n_obs_steps"] = cfg.n_obs_steps
            else:
                rl_cfg.n_obs_steps = cfg.n_obs_steps
            rl_runner = RLRolloutRunner(
                rl_cfg,
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                train_losses = list()
                if training_stage in ("bc", "joint"):
                    with tqdm.tqdm(
                            train_dataloader,
                            desc=f"BC epoch {self.epoch}",
                            leave=False,
                ...
                if rl_enabled and training_stage in ("rl", "joint"):
                    self.model.eval()
                    rollout_log = rl_runner.collect(self.model)
                    self.model.train()
                    step_log.update(rollout_log)
                    rl_anchor_iter = iter(train_dataloader)
                    rl_updates = int(rl_cfg.get("updates_per_epoch", 1))
                    rl_batch_size = int(rl_cfg.get("batch_size", cfg.dataloader.batch_size))
                    rl_update_count = 0
                    for rl_batch in rl_runner.iter_batches(rl_batch_size, device):
                        if rl_update_count >= rl_updates:
                            break
                        try:
                            raw_bc_batch = next(rl_anchor_iter)
                        except StopIteration:
                            rl_anchor_iter = iter(train_dataloader)
                            raw_bc_batch = next(rl_anchor_iter)
                        bc_batch = dataset.postprocess(raw_bc_batch, device)
                        if train_sampling_batch is None:
                            train_sampling_batch = bc_batch
                        raw_loss = self.model.compute_rl_loss(rl_batch, bc_batch=bc_batch)
                        loss = raw_loss / cfg.training.gradient_accumulate_every
                        loss.backward()
