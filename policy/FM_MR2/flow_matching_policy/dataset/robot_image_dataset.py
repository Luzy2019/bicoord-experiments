...
from flow_matching_policy.common.normalize_util import get_image_range_normalizer
import pdb
...
class RobotImageDataset(BaseImageDataset):
    ...
    def _sample_to_data(self, sample):
        ...
        left_cam = sample["left_camera"] / 255
        right_cam = sample["right_camera"] / 255
    ...
    def postprocess(self, samples, device, n_obs_steps=None):
        obs_slice = slice(None, n_obs_steps) if n_obs_steps is not None else slice(None)
        agent_pos = samples["state"][:, obs_slice].to(device, non_blocking=True)
        head_cam = samples["head_camera"][:, obs_slice].to(device, non_blocking=True) / 255.0
        left_cam = samples["left_camera"][:, obs_slice].to(device, non_blocking=True) / 255.0
        right_cam = samples["right_camera"][:, obs_slice].to(device, non_blocking=True) / 255.0
