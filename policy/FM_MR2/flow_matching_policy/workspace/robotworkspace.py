...
def _make_wandb_log(step_log, prefix="train"):
    log = {}
    for key, value in step_log.items():
        ...
        if scalar is None:
            continue
        if key == "train_loss":
            log[f"{prefix}/loss"] = scalar
        elif key == "val_loss":
            log["val/loss"] = scalar
        elif key == "train_action_mse_error":
            log["metrics/train_action_mse_error"] = scalar
        elif key.startswith("val_"):
            log[f"val/{key[4:]}"] = scalar
        elif key in ("global_step", "epoch"):
            log[key] = scalar
        elif key == "lr":
            log[f"{prefix}/lr"] = scalar
        else:
            log[f"{prefix}/{key}"] = scalar
    return log
...
class RobotWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        wandb.config.update(
            {
                ...
                "val_batches_per_epoch": len(val_dataloader),
        ...
        wandb_run.define_metric("metrics/*", step_metric="global_step")
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        batch = dataset.postprocess(batch, device, n_obs_steps=cfg.n_obs_steps)
                        ...
                        if not is_last_batch:
                            ...
                            wandb.log(
                                _make_wandb_log(step_log, prefix="train"),
                    ...
                    with torch.no_grad():
                        ...
                        with tqdm.tqdm(
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                ...
                                if hasattr(self.model, "last_loss_dict"):
                                    for key, value in self.model.last_loss_dict.items():
                                        scalar = _as_wandb_scalar(value)
                                        if scalar is not None:
                                            step_log.setdefault(f"val_{key}", []).append(scalar)
                        ...
                        for key, value in list(step_log.items()):
                            if key.startswith("val_") and isinstance(value, list) and len(value) > 0:
                                step_log[key] = float(np.mean(value))
                ...
                # checkpoint
                if cfg.training.checkpoint_every > 0 and ((self.epoch + 1) % cfg.training.checkpoint_every) == 0:
                    # checkpointing
